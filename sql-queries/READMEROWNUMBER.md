...
WITH a AS (
    SELECT *,
           ROW_NUMBER() OVER(PARTITION BY address_hash ORDER BY verified_at) AS rn
    FROM addresses
)
SELECT *
FROM a
...
WITH d AS (
    SELECT *,
           ROW_NUMBER() OVER(PARTITION BY device_id ORDER BY log_time DESC) AS rn
    FROM device_logs
)
SELECT *
FROM d
...
Section 2 — Top-N Per Group
Q11: Top 3 orders per customer by amount (Amazon)
WITH o AS (
    SELECT *,
...
Q12: Top 2 highest-rated movies per genre (Netflix)
WITH m AS (
    SELECT *,
...
Q13: Top 5 most viewed posts per creator (Instagram)
WITH p AS (
    SELECT *,
...
Q14: Top 1 earning driver per city (Uber)
WITH d AS (
    SELECT *,
...
Q15: Top 10 bestselling products per category (Amazon Marketplace)
WITH p AS (
    SELECT *,
...
Q16: Top 3 busiest stations per city (Lyft)
WITH s AS (
    SELECT *,
           ROW_NUMBER() OVER(PARTITION BY city ORDER BY rides DESC) AS rn
    FROM station_usage
)
SELECT *
FROM s
...
Q17: Find the most active volunteer per event (Airbnb Social Impact)
WITH v AS (
    SELECT *,
           ROW_NUMBER() OVER(PARTITION BY event_id ORDER BY hours DESC) AS rn
    FROM volunteers
)
SELECT *
FROM v
...
Q18: Top 2 highest scoring students per class (Google Education)
WITH r AS (
    SELECT *,
...
Q19: Return only the longest call per user (Zoom)
WITH c AS (
    SELECT *,
...
Q20: Find top restaurant per cuisine by rating (Yelp)
WITH r AS (
    SELECT *,
           ROW_NUMBER() OVER(PARTITION BY cuisine ORDER BY rating DESC) AS rn
    FROM restaurants
)
SELECT *
FROM r
...
Section 3 — First & Last Records
Q21: Get first purchase of each customer (Amazon)
WITH t AS (
    SELECT *,
...
Q22: Get last login per user (Meta)
WITH l AS (
    SELECT *,
...
Q23: First version of each document (Google Docs)
WITH v AS (
    SELECT *,
           ROW_NUMBER() OVER(PARTITION BY doc_id ORDER BY version_date) AS rn
    FROM doc_versions
)
SELECT *
FROM v
...
Q24: Latest salary record per employee (LinkedIn)
WITH s AS (
    SELECT *,
...
Q25: Most recent subscription plan per user (Spotify)
WITH p AS (
    SELECT *,
...
Q26: First country visited per traveler (Airbnb)
WITH t AS (
    SELECT *,
           ROW_NUMBER() OVER(PARTITION BY traveler_id ORDER BY visit_date) AS rn
    FROM trips
)
SELECT *
FROM t
...
Q27: Last comment before closing a ticket (Zendesk)
WITH c AS (
    SELECT *,
           ROW_NUMBER() OVER(PARTITION BY ticket_id ORDER BY comment_time DESC) AS rn
    FROM comments
)
SELECT *
FROM c
...
Q28: Get first job title of each employee (Workday)
WITH j AS (
    SELECT *,
...
Q29: Latest product price (Amazon Retail)
WITH p AS (
    SELECT *,
...
Q30: First failed login attempt per user (Security)
WITH f AS (
    SELECT *,
